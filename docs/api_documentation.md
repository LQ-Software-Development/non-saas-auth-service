...
- **Autenticação:** Requer JWT (`Authorization: Bearer <token>`). O uso do token de acesso aqui é incomum para uma operação de _refresh_, que normalmente usa o _refresh token_ em si. **Verificar implementação!**
- **Possíveis Respostas:**
...
- **Parâmetros de Rota:**
  - `organizationId`: ID da organização.
  - `id`: ID do participante (geralmente o ID da _relação_ participante-organização, não o ID do usuário). **Confirmar estrutura!**
- **Possíveis Respostas:**
...
- **Descrição:** Adiciona um ou mais participantes a uma organização. Suporta configuração opcional de comissões através do campo `metadata`.
- **Autenticação:** Requer Header `application-key`.
- **Parâmetros de Rota:**
  - `organizationId`: ID da organização.
- **Corpo da Requisição:** Array de `CreateParticipantDto`.
  - **Campo `metadata` (opcional):** Objeto com configurações adicionais do participante, incluindo configuração de comissões.
  - **Estrutura de `metadata.commissionConfig` (opcional):**
    ```json
    {
      "name": "João Silva",
    ...
    ```
  - **Campos de `commissionConfig`:**
    - `type` (opcional, padrão: "percentage"): Tipo de comissão - `"percentage"` ou `"fixed"`.
    - `baseCommission` (opcional): Valor da comissão base (número).
    - `newClientCommission` (opcional): Valor da comissão para novos clientes (número).
    - `rules` (opcional): Regras de cálculo da comissão:
      - `calculationBase` (opcional, padrão: "paid_sale"): Base de cálculo - `"paid_sale"` ou `"total_sale"`.
      - `deductAsaasFees` (opcional, padrão: false): Se deve deduzir taxas do gateway.
      - `transferClientAfterInactivation` (opcional, padrão: true): Se transfere cliente após inativação.
      - `notifyBeforeInactivation` (opcional, padrão: false): Se notifica antes de inativar.
    - `paymentInfo` (opcional): Informações de pagamento:
      - `account`: Conta para pagamento (string).
      - `gateway`: Gateway de pagamento usado (string livre, ex: "asaas", "stripe").
    - `penalties` (opcional): Valor de multa (número).
  - **Observações sobre `commissionConfig`:**
    - Este campo é **completamente opcional** e não afeta projetos que não utilizam configuração de comissões.
    - Defaults são aplicados automaticamente apenas quando `commissionConfig` está presente.
    - Validação é ativada somente quando o campo é enviado.
    - O campo `gateway` aceita qualquer string para flexibilidade entre diferentes projetos.
- **Possíveis Respostas:**
  - `201 Created`: Participantes adicionados com sucesso.
  - `400 Bad Request`: Dados inválidos (incluindo validação de `commissionConfig` se presente).
  - `401 Unauthorized`/`403 Forbidden`: Chave de aplicação inválida ou ausente.
  - `404 Not Found`: Organização não encontrada.
  - `500 Internal Server Error`: Erro interno.
...
- **Descrição:** Lista todos os participantes de uma organização, incluindo suas configurações de `metadata` (como `commissionConfig`).
- **Autenticação:** Requer Header `application-key`.
- **Parâmetros de Rota:**
  - `organizationId`: ID da organização.
- **Possíveis Respostas:**
  - `200 OK`: Retorna lista de participantes com seus respectivos metadados.
  - `401 Unauthorized`/`403 Forbidden`: Chave de aplicação inválida ou ausente.
  - `404 Not Found`: Organização não encontrada.
  - `500 Internal Server Error`: Erro interno.

#### 3. Atualizar Participante (Admin)

- **Endpoint:** `PATCH /admin/organizations/:organizationId/participants/:id`
- **Descrição:** Atualiza informações de um participante específico. Suporta atualização incremental de `metadata.commissionConfig` através de deep merge.
- **Autenticação:** Requer Header `application-key`.
- **Parâmetros de Rota:**
  - `organizationId`: ID da organização (não utilizado na lógica atual, mas presente na rota).
  - `id`: ID do participante a ser atualizado.
- **Corpo da Requisição:** `UpdateParticipantDto` (campos parciais).
  - **Atualização de `metadata.commissionConfig` (opcional):**
    - Permite atualização parcial (ex: enviar apenas `rules.deductAsaasFees: true` sem reenviar toda estrutura).
    - Faz merge profundo com valores existentes.
    - Aplica defaults apenas aos novos campos enviados.
    ...
    ```json
    {
      "metadata": {
        "commissionConfig": {
          "baseCommission": 18,
          "rules": {
            "deductAsaasFees": true
          }
        }
      }
    }
    ```
    (Mantém valores existentes de `type`, `newClientCommission`, `paymentInfo`, etc.)
- **Possíveis Respostas:**
  - `200 OK`: Participante atualizado com sucesso.
  - `400 Bad Request`: Dados inválidos.
  - `401 Unauthorized`/`403 Forbidden`: Chave de aplicação inválida ou ausente.
...
- A rota `POST /auth/refresh-token-info` usa autenticação JWT baseada no token de _acesso_ para uma operação de _refresh_, o que é atípico. Verificar a lógica no `RefreshTokenInfoService`.
